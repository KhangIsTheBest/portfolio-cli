...
export const ThemeSelector: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <Moon className={`w-3.5 h-3.5 text-[var(--primary-color)] z-10 mr-0.5 transition-opacity duration-300 ${isDark ? 'opacity-100 font-bold' : 'opacity-40'}`} />

      {/* Sliding thumb pill */}
      <span
        className={`absolute top-1 left-1 w-6 h-6 rounded-full shadow-md transition-transform duration-300 ease-out transform ${
          isDark 
            ? 'translate-x-6 bg-[var(--primary-bg)] border border-[var(--primary-border)]' 
